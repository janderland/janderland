...
isolation or composed together into a larger transaction.
...
Let's focus on the case where two clients are trying to
claim the same unlocked mutex. Both clients will read the
empty owner key `/path/to/mutex("owner","")`. They will both
try to clear this key and replace it with one containing
their respective names.

The transaction keeps track of a [conflict range][] which
includes all the key-values read by the client. When it
comes time to commit the transaction, Foundation DB checks
if any of the key-values in the conflict range were
modified. If so, the transaction is aborted and retried.

[conflict range]: https://apple.github.io/foundationdb/developer-guide.html#conflict-ranges

![Transaction Sequence Diagram](img/seq.svg)

In the sequence diagram above, take a look at the events
happening between start and subsequent failure of
`client-A`'s first transaction. Because `client-A` read the
empty owner key, that key becomes a part of it's conflict
range. `client-B` overwrites this key and commits before
`client-A` commits. This causes `client-A`'s transaction to
be rejected. Upon retrying the transaction, `client-A` sees
that `client-B` is now the owner of the mutex and places
itself onto the queue.
