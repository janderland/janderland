...
`/my/dir/lath` could be mapped to the prefix
`0xfa3209`. Key-values written into this directory would
include this prefix.
...
keeps changing with new versionstamps, we know the mutex
owner is still alive. As the queue index, because
...
> which I have yet to document publicly. In brief, there are
> a couple of changes. `vstamp` is a new data type
> representing a versionstamp. `clear` works on ranges of
> keys.
...
an argument. This can either be the handle to the database
or a running transaction, allowing the queries to be run in
...
tracks the key-values written & read during the transaction.
When it comes time to commit, Foundation DB checks if any of
the key-values read were modified by another client during
the transaction. If so, the transaction is rolled back and
...
```language-go
func TestOwnerMarshaling(t *testing.T) {
  t.Run("key", func(t *testing.T) {
      in := "name"
      out := UnmarshalOwnerKey(MarshalOwnerKey(in))
      require.Equal(out, in)
  })
  t.Run("value", func(t *testing.T) {
      in := []byte{0xf2,0x43,0xac,0x00}
      out := UnmarshalOwnerValue(MarshalOwnerValue(in))
      require.Equal(out, in)
  }
}
```
...
[forums][]. I'll do my best to answer any questions.
