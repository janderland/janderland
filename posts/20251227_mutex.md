---
title: Intro to FoundationDB via a Distributed Mutex
date: 2025-12-27
languages: [fql, go]
...
...
implement a distributed mutex on top of FoundationDB. 

Calling FoundationDB the "assembly language of databases"
(something I once read on [Hacker News][]) is an
exaggeration. Nonetheless, FoundationDB does provide a set of
...
## Intro to FoundationDB

[FoundationDB][] is a distributed key-value database with
automatic shard and replication management. Keys are byte
strings (under 10kB) pointing at values which are slightly
larger byte strings (under 100kB). FoundationDB is designed
...
FoundationDB to guarantee [ACID][] properties. There's
a catch though: [consistency][] (the 'C' in ACID) is
enforced by the clients. FoundationDB does not ensure data
...
FoundationDB sorts its key-values by the numeric value of
...
FoundationDB provides a special kind of subspace called
...
FoundationDB provides an encoding scheme for [tuples][].
...
As stated earlier, FoundationDB does nothing to enforce the
...
a better understanding of FoundationDB's concurrency model.
...
For correct behavior, FoundationDB will need to manage
...
transaction. When it comes time to commit, FoundationDB
...
an containerized FoundationDB cluster. I reimplemented this
...
installation of FoundationDB works fine as well.
...
FoundationDB doesn't enforce the schema, so the extra
...
As the name implies, FoundationDB operates at a lower level
...
FoundationDB can partially replace common patterns like
...
the FoundationDB [documentation][] for more information. If
you have any questions, drop by the FoundationDB
