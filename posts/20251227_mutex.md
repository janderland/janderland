...
DB is design for highly concurrent workloads with many
...
The schema for our mutex is fairly simple. We will have
a key-value for identifying the owner of the mutex. We'll
also need a queue on which waiting actors may place
themselves. When the owner of the mutex releases, it will
grab the next owner from the head of the queue. Clients can
watch the key-value specifying the owner and react when they
are given control of the mutex.
